...
from WriteHDLUtils import getHLSMemoryClassName, groupAllConnectedMemories, writeTBMemoryStimulusInstance, writeTBMemoryReadInstance, writeTopLevelMemoryInstance, writeControlSignals_interface, writeMemoryLHSPorts_interface, writeMemoryRHSPorts_interface, writeTBControlSignals, writeFWBlockControlSignals, writeFWBlockMemoryLHSPorts, writeFWBlockMemoryRHSPorts, writeModuleInstance
...
def writeMemoryModules(mem_list, interface=0):
    ...
    for memModule in sorted(mem_list,key=lambda x: x.index):
        amem_str,_ = writeTopLevelMemoryInstance(memModule,interface)
        string_mem += amem_str
...
def writeFWBlockInstance(topfunc, memories_in, memories_out, first_proc, last_proc):
    ...
    string_fwblock_ctrl = writeFWBlockControlSignals(first_proc, last_proc)
...
def writeTestBench(topfunc, memories_in, memories_out, emData_dir, sector="04"):
    ...
    string_header = "// Test bench generated by generator_verilog.py\n\n"
    string_header += "`timescale 1ns / 1ps\n\n"
    string_header += "module " + topfunc + "_test();\n\n"
